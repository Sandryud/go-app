...
Дополнительно для **share** (миграция `000006_workout_plan_shares_and_copy_events`):

- **`workout_plan_shares`** — публичная ссылка на план: `plan_id`, уникальный `token`, `revoked_at` (NULL = ссылка активна). Не более одной активной ссылки на план (частичный уникальный индекс по `plan_id` при `revoked_at IS NULL`).
- **`workout_plan_copy_events`** — событие «забрать план себе» по share: `source_plan_id`, `copy_plan_id`, `recipient_user_id`, `channel = 'share'`, `share_id`, для метрик (`total_copies`, уникальные получатели).
...
Создать или получить **активную** share-ссылку на план (для соцсетей / ленты). Только **владелец** плана.

**Ответ:** `200 OK` — объект:

| Поле | Тип | Описание |
|------|-----|----------|
| share_token | string (UUID) | Токен для пути публичного API |
| share_path | string | Относительный путь, напр. `/api/v1/public/plans/by-share/{token}` |
| share_url | string | Полный URL, если задан `PUBLIC_BASE_URL` в конфиге сервера; иначе поле опущено |

**Ошибки:** 400 (неверный UUID плана), 401, 403, 404 (план не найден), 500.

---

#### DELETE /api/v1/plans/:id/share

Отозвать активную share-ссылку. Только владелец.

**Ответ:** `204 No Content`.

**Ошибки:** 400, 401, 403, 404 (`plan_not_found` или `share_not_found`, если активной ссылки не было), 500.

---

#### GET /api/v1/plans/:id/share/stats

Метрики копирований плана через share: сколько раз скопировали и сколько **уникальных** пользователей забрали план. Только владелец.

**Ответ:** `200 OK` — `{ "total_copies": number, "unique_recipients": number }`.

**Ошибки:** 400, 401, 403, 404, 500.

---

#### GET /api/v1/public/plans/by-share/:token

Публичный просмотр структуры плана по **активному** токену. **Без авторизации.**

Структура ответа как у `GET /api/v1/plans/:id` (план с `days` и `exercises`); поле `is_active` в ответе всегда `false` (не раскрывать состояние владельца). Идентификаторы в дереве — от **исходного** плана (для отображения); после копирования у получателя будут новые UUID.

**Ошибки:** 400 (неверный token), 404 (`share_not_found`), 500.

---

#### POST /api/v1/public/plans/by-share/:token/copy

Создать **глубокую копию** плана в аккаунте текущего пользователя. Требуется **JWT**. Копия: `is_active = false`, `is_public = false`, `source_plan_id` = исходный план; событие пишется в `workout_plan_copy_events` для метрик.

**Тело запроса (JSON, опционально):** `{ "name": "string" }` — имя копии (макс. 200 символов); если тело пустое или поле не передано — имя вида «Копия: {исходное имя}».

**Ответ:** `201 Created` — объект плана без дней (как при `POST /api/v1/plans`): `id`, `user_id`, `name`, `is_active`, `is_public`, `category`, `level`, `created_at`, `updated_at`.

**Ошибки:** 400 (`cannot_copy_own_plan` — нельзя скопировать свой план по своей ссылке), 401, 404 (`share_not_found`), 500.
...
| 404 | share_not_found | Share-ссылка не найдена, отозвана или план удалён |
| 400 | cannot_copy_own_plan | Нельзя скопировать свой план по своей share-ссылке |
...
- **Share:** публикация в каталоге по `is_public` остаётся отдельным сценарием (admin); share позволяет любому автору поделиться планом по ссылке без выставления `is_public`.
